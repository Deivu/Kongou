...
export class PrimaryIpc {
	...
	constructor(manager: Manager) {
		...
		this.manager.indomitable.on('message', message => void this.onMessage(message as IpcMessage));
	}
	...
	public async send(id: number, content: IpcContent, repliable = false): Promise<unknown> {
	...
	public async broadcast(content: IpcContent, repliable = false): Promise<unknown> {
		const controller = new AbortController();
		const timeout = setTimeout(() => controller.abort(), 30000).unref();
