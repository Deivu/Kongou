...
class KongouEvent extends  EventEmitter {
    constructor(client) {
        ...
        if (this.run === undefined) throw new TypeError('Classes extending KongouEvent must implement an async function "run"');
        if (this.run.constructor.name !== 'AsyncFunction') throw new TypeError('Classes extending KongouEvent must implement "run" as async function');
